...
class Like(models.Model):
    ...
    Fields:
    - owner: The user who liked the post, linked as a foreign key to the
      User model.
             If the user is deleted, their likes will also be removed-
             (on_delete=models.CASCADE).
    - post: The post that has been liked, linked as a foreign key to the
            Post model.
            Includes a 'related_name' attribute ('likes') to enable reverse
            access from a Post instance.
    - created_at: A timestamp that indicates when the like was created,
